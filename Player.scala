...
class Player(startingArea: Area):
  ...
  def drop(itemName:String): String =
    val attempToDrop = playerItems.remove(itemName)
    attempToDrop match {
      case Some(item) =>
  ...
  def examine(itemName: String): String =
    val attempToExamine = playerItems.get(itemName)
    attempToExamine match {
      case Some(item) => s"You look closely at the ${item}.\n${item.description}"
      case None => s"If you want to examine something, you need to pick it up first."
    }

  def get(itemName:String): String =
    val attempToGet = currentLocation.removeItem(itemName)
    attempToGet match {
      case Some(item) =>
        this.playerItems += itemName->item
        s"You pick up the ${item}."
      case None => s"There is no ${itemName} here to pick up."
    }

  def has(itemName: String): Boolean =
    val itemChecker = this.playerItems.get(itemName)
    itemChecker.isDefined

  /** Determines if the player has indicated a desire to quit the game. */
  def hasQuit = this.quitCommandGiven

  def inventory: String =
    if playerItems.nonEmpty then
     s"You are carrying:\n${this.playerItems.keys.toList.mkString("\n")}"
    else
